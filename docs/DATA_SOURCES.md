...
| Automation strategy features | Automation desired-state signals, reason codes, config-hashed feature snapshots, proof snapshots, allocator snapshots, reconciliation, churn/forward validation anchors | Derived from thesis state + daily OHLCV technical state plus existing risk/portfolio/open-position/sleeve state | n/a | `strategy-runner` reads `automation_trade_permission`, `automation_strategy_definition`, `thesis`, `price_bar`, risk config, portfolio settings, allocation policy, sleeves, and broker-position aggregates | wired - `technical_timing@0.1.0` and `thesis_timing@0.1.0` run deterministic proof policy first; blocked preflights append `automation_proof` only, while passed/warning evaluations emit append-only `desired_strategy_position`, `automation_proof`, sleeve allocation updates, reconciliation, and `automation_strategy_signal_observation`; shadow scopes receive digital simulator fills, while paper/live scopes stop at `automation_execution_reconciliation.status='needs_order'` for a separate broker adapter |
| Automation readiness scoring | Forward-only promotion gates for strategy lifecycle stages, operator approvals, proof pass rate, incidents, paper fill quality, churn, drawdown, and baseline comparison | Derived from `automation_strategy_signal_observation`, `automation_proof`, `automation_incident`, `automation_broker_order`, and operator approval rows | n/a | `automation-readiness` writes `automation_strategy_readiness_evaluation` and lifecycle events; `/api/automation/status` exposes latest readiness per permission | wired - promotions require readiness thresholds plus an approved, unexpired `automation_strategy_promotion_approval`; failed live-capable readiness can set `manual_freeze` on approved canary/expanded permissions |
| Technical timing validation | Forward-only outcomes for `pullback_reversal`, `pullback_watch`, `constructive_trend`, `extended_chase`, and `avoid_breakdown`; return, drawdown, benchmark return, excess return, win/outperform precision | Derived from point-in-time `TechnicalState` snapshots and future daily OHLCV bars | n/a | `/api/technical-state` records `technical_timing_observation` rows when a tracked state is observed; `technical-timing-validator` scores due rows once enough future daily bars exist | wired - preserves input snapshots, compares against QQQ/SMH benchmark bars when available, and exposes grouped calibration in `/api/calibration` |
...
| Paper order ids/status/events | First broker write path for approved paper automation; bracket parent/child ids, fills, rejects, cancels, errors | **IBKR paper** via `ib_insync` against TWS/IB Gateway | paper trading free | wired but disabled — `py/src/stocks/ibkr_paper_orders.py` consumes `automation_execution_reconciliation.status='needs_order'` rows only when `automation_paper_order_config.enabled=true`, `broker_account` is `DU...`, and `IBKR_PAPER_ORDERS_ENABLED=true`; records `automation_broker_order` plus append-only `automation_broker_order_event`; rejects live-looking API ports and non-paper automation scopes; run one guarded pass with `make ibkr-paper-orders` |
