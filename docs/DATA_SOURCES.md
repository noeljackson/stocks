...
| Shadow strategy features | Automation desired-state signals, reason codes, config-hashed feature snapshots, churn/forward validation anchors | Derived from thesis state + daily OHLCV technical state | n/a | `strategy-runner` reads `automation_trade_permission`, `automation_strategy_definition`, `thesis`, and `price_bar` | wired - shadow-only `technical_timing@0.1.0` and `thesis_timing@0.1.0` emit append-only `desired_strategy_position`, `automation_proof`, and `automation_strategy_signal_observation`; no broker orders or executable reconciliation |
